  ...
  return (
    <section id="portfolio" className="py-24 bg-[#050505] relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.2 }} className="flex flex-wrap gap-2 mb-10">
          {categories.map((cat) => (
            <button
              key={cat}
              onClick={() => setActive(cat)}
              className="px-4 py-2 text-sm font-medium rounded-full transition-all duration-300"
              style={active === cat ? { background: 'linear-gradient(135deg, #7c3aed, #a855f7)', color: 'white', boxShadow: '0 0 20px rgba(124,58,237,0.4)' } : { background: 'rgba(18,18,26,0.7)', color: '#9ca3af', border: '1px solid rgba(168,85,247,0.15)' }}
            >
              {cat}
            </button>
          ))}
        ...
        <AnimatePresence mode="wait">
          <motion.div key={active} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {filtered.map((work, i) => (
